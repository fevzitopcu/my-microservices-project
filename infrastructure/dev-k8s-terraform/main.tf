...
resource "aws_instance" "kube-master" {
    ...
    iam_instance_profile = module.iam.master_profile_name.value
    vpc_security_group_ids = [aws_security_group.matt-kube-master-sg.id, aws_security_group.matt-kube-mutual-sg.id]
    key_name = "mattkey"  # job çalıştırırken değiştirilecek
    subnet_id = "subnet-0470b583b72022b6a"  # select own subnet_id of us-east-1a
...
resource "aws_instance" "worker-1" {
    ami = "ami-013f17f36f8b1fefb"
    instance_type = "t3a.medium"
    iam_instance_profile = module.iam.worker_profile_name.value
